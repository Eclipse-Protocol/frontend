...
function Deploy() {
  ...
  const validationError = useMemo(() => {
    if (!form.name.trim()) return "Strategy name is required.";
    if (form.name.trim().length < 3) return "Strategy name must be at least 3 characters.";
    if (!form.desc.trim()) return "Description is required.";
    if (!form.bond.trim()) return "Bond amount is required.";
    const parsedBond = Number(form.bond);
    if (isNaN(parsedBond) || parsedBond <= 0) return "Bond amount must be a positive number.";
    return null;
  }, [form]);

  function generateShellScript() {
    const maxPosBps = form.maxPosition * 100;
    const maxDrawdownBps = form.drawdown * 100;
    const safeName = (form.name || "strategy").toLowerCase().replace(/[^a-z0-9]/g, "-");

    return `#!/usr/bin/env bash
# deployment-scripts/deploy-${safeName}.sh
# Generated by Eclipse Protocol Strategist Wizard on ${new Date().toISOString()}

# Ensure you are in the contracts/ directory of the repository
# Fill in your Private Key (must hold C2FLR for gas) and the desired fee recipient addresses
export PRIVATE_KEY="0xYOUR_PRIVATE_KEY_HERE"
export TREASURY_ADDRESS="0xYOUR_TREASURY_ADDRESS_HERE"
export STRATEGIST_ADDRESS="${address || "0xYOUR_STRATEGIST_ADDRESS_HERE"}"

# Infrastructure & Registry Parameters (Coston2 Testnet defaults)
export VAULT_ASSET_ADDRESS="${VAULT_ASSET_ADDRESS}"
export DEX_ROUTER_ADDRESS="0x9B1b742477ECf3810e2184De0959e9C75268aD21"
export FTSO_V2_ADDRESS="0xC4e9c78EA53db782E28f28Fdf80BaF59336B304d"
export FDC_VERIFICATION_ADDRESS="0x906507E0B64bcD494Db73bd0459d1C667e14B933"
export BOND_TOKEN_ADDRESS="0x88111bF10C59d828F20760F2604E833eB9c452e8"

# Strategy Specific Parameters (Drafted from Wizard)
export STRATEGY_NAME="${form.name || "Eclipse Strategy"}"
export MAX_POSITION_SIZE_BPS="${maxPosBps}"
export MAX_DRAWDOWN_BPS="${maxDrawdownBps}"
export BOND_AMOUNT="${form.bond || "1000"}"

echo "Deploying strategy: $STRATEGY_NAME..."
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-3xl px-6 pt-12">
        ...
        <div className="mt-4 rounded-lg border border-eclipse-gold/30 bg-eclipse-gold/5 p-3 text-xs text-eclipse-text font-sans">
        ...
        <div className="glass-card mt-6 p-6">
          {step === 0 && (
            <div className="space-y-4">
              ...
              {validationError && (
                <div className="text-xs text-eclipse-danger font-medium mt-2">
                  ⚠️ {validationError}
                </div>
              )}
            </div>
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div className="mt-4 border-t border-eclipse-border pt-4">
                <div className="mb-2 flex items-center justify-between text-xs font-semibold uppercase tracking-wider text-eclipse-text">
                  <span>Generated Shell Deployment Script</span>
                  <button
                    onClick={() => copyText(generateShellScript(), "Deployment script")}
                    className="inline-flex items-center gap-1 text-eclipse-purple hover:underline"
                  >
                    <Copy className="h-3.5 w-3.5" /> Copy Script
                  </button>
                </div>
                <pre className="overflow-x-auto rounded-lg border border-eclipse-border bg-black/60 p-4 font-mono text-[11px] leading-relaxed text-eclipse-muted">
                  <code>{generateShellScript()}</code>
                </pre>
              </div>

              <div className="mt-6 rounded-lg border border-eclipse-purple/30 bg-eclipse-purple/5 p-4 space-y-3 font-sans">
                <h4 className="font-semibold text-xs text-eclipse-text uppercase tracking-wider">What happens next (Manual Strategy Setup)</h4>
                <ol className="list-decimal pl-4 space-y-2 text-xs text-eclipse-muted leading-relaxed">
                  <li>
                    <strong className="text-eclipse-text">Download files:</strong> Download both the configuration draft and the shell deployment script. Place both files inside the <code className="font-mono bg-eclipse-bg/60 px-1 rounded text-eclipse-purple">contracts/</code> directory.
                  </li>
                  <li>
                    <strong className="text-eclipse-text">Deploy:</strong> Edit <code className="font-mono bg-eclipse-bg/60 px-1 rounded text-eclipse-purple">deploy-{(form.name || "strategy").toLowerCase().replace(/[^a-z0-9]/g, "-")}.sh</code> with your private key and execute it to run the deployment scripts:
                    <pre className="mt-1 rounded bg-black/40 p-2 font-mono text-[10px]">
                      bash deploy-{(form.name || "strategy").toLowerCase().replace(/[^a-z0-9]/g, "-")}.sh
                    </pre>
                  </li>
                  <li>
                    <strong className="text-eclipse-text">Register:</strong> Once the TEE has booted up and the relayer obtains the Web2Json attestation proof from the Flare Data Connector, execute the registration script on-chain:
                    <pre className="mt-1 rounded bg-black/40 p-2 font-mono text-[10px] whitespace-pre-wrap">
{`export PRIVATE_KEY="0xYOUR_PRIVATE_KEY"
export ENCLAVE_REGISTRY_ADDRESS="<deployed_registry_address>"
export ALPHA_VAULT_ADDRESS="<deployed_vault_address>"
export ENCLAVE_SIGNER_ADDRESS="<enclave_signer_address>"
export FDC_ATTESTATION_PROOF="<proof_bytes>"
forge script script/RegisterEnclave.s.sol:RegisterEnclave \\
  --rpc-url https://coston2-api.flare.network/ext/C/rpc \\
  --broadcast`}
                    </pre>
                  </li>
        ...
        <div className="mt-6 flex justify-between">
          ...
          {step < steps.length - 1 ? (
            <button
              ...
              className="rounded-lg bg-eclipse-purple px-5 py-2 text-sm font-medium text-white glow-purple hover:bg-eclipse-purple-bright disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Continue
            </button>
          ) : (
            <div className="flex gap-3">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg border border-eclipse-border px-5 py-2 text-sm font-medium text-eclipse-text hover:border-eclipse-purple/60"
              >
                <Download className="h-4 w-4" /> Download shell script
              </button>
              <button
      ...
      {/* Section Separator */}
      <div className="mx-auto my-12 max-w-3xl px-6">
        <div className="border-t border-eclipse-border/80 my-8" />
      </div>

      {/* Real lifecycle status of the one vault that actually exists — not gated on the wizard above */}
      <div className="mx-auto max-w-3xl px-6 pb-20">
        <div className="rounded-lg border border-eclipse-teal/30 bg-eclipse-teal/5 p-4 mb-6 font-sans">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-semibold text-eclipse-teal uppercase tracking-wider flex items-center gap-2">
                <span className="h-2.5 w-2.5 rounded-full bg-eclipse-teal animate-pulse" />
                Live Coston2 Protocol Deployment Status
              </h3>
              <p className="mt-1 text-xs text-eclipse-muted">
                This is the real-time status of the active, production-grade vault currently live on Flare Coston2. It is completely independent of the configuration form draft above.
              </p>
            </div>
            <span className="rounded bg-eclipse-teal/15 px-2 py-1 font-mono text-[10px] text-eclipse-teal border border-eclipse-teal/20">
              Live Vault
            </span>
          </div>
        </div>
