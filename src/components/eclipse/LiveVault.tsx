...
function useAlphaVaultCore() {
  ...
  const maxPositionSizeBps = useReadContract({
    ...ALPHA_VAULT,
    functionName: "maxPositionSizeBps",
  });
  ...
  const assetToken = assetAddress.data
    ? ({ address: assetAddress.data, abi: ERC20_ABI } as const)
    : undefined;
  ...
  const assetsNum =
    assetDecimals.data !== undefined
      ? Number(formatUnits(totalAssets.data ?? 0n, assetDecimals.data))
      : undefined;
  const sharesNum =
    shareDecimals.data !== undefined
      ? Number(formatUnits(totalSupply.data ?? 0n, shareDecimals.data))
      : undefined;
  const pricePerShare =
    assetsNum !== undefined && sharesNum !== undefined && sharesNum > 0 ? assetsNum / sharesNum : 1;
...
export function LiveStatCards() {
  const v = useAlphaVaultCore();
  const tvl =
    v.assetDecimals.data !== undefined
      ? fmtUSD(Number(formatUnits(v.totalAssets.data ?? 0n, v.assetDecimals.data)))
      : "—";
  ...
  const drawdown =
    v.maxDrawdownBps.data !== undefined
      ? `${(Number(v.maxDrawdownBps.data) / 100).toFixed(2)}%`
      : "—";
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
      <StatCard
        label="TVL (totalAssets)"
        value={tvl}
        hint={v.assetSymbol.data ? `in ${v.assetSymbol.data}` : undefined}
      />
      ...
      <StatCard
        label="Strategist bond"
        value={bond}
        hint="StrategyRegistry · at risk of slashing"
      />
      <StatCard
        label="Max drawdown (config)"
        value={drawdown}
        hint={v.paused.data ? "Vault paused" : "Circuit breaker"}
      />
    </div>
  );
}
  ...
  return (
    <dl className="mt-4 grid gap-4 sm:grid-cols-2">
      ...
      <Row
        k="Max position size"
        v={
          v.maxPositionSizeBps.data !== undefined
            ? `${(Number(v.maxPositionSizeBps.data) / 100).toFixed(2)}% of TVL`
            : "—"
        }
      />
      <Row
        ...
        v={
          v.maxDrawdownBps.data !== undefined
            ? `${(Number(v.maxDrawdownBps.data) / 100).toFixed(2)}% halts trading`
            : "—"
        }
      />
...
export function LivePerformanceLedger() {
  // NAV is committed as totalAssets() — the underlying's own decimals (6 on Coston2's Mock USDC),
  // not a fixed 18-decimal WAD value — so it must be formatted with the asset's real decimals or
  // every row understates NAV by 10^12x.
  const { assetDecimals } = useAlphaVaultCore();
  ...
  if (epochCount.isLoading) {
    return (
      <div className="px-5 py-8 text-center text-sm text-eclipse-muted">Reading epoch history…</div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center gap-2 px-5 py-10 text-center">
        ...
        <p className="max-w-sm text-xs text-eclipse-muted">
          ...
          <code className="mx-1 rounded bg-eclipse-bg/60 px-1 py-0.5 text-eclipse-purple">
            commitEpoch()
          </code>
          starts running, entries will appear here, hash-chained back to genesis.
        </p>
        <div className="mt-2 font-mono text-[11px] text-eclipse-muted">
          Genesis hash: {shortHash(genesisHash.data)}
        </div>
      </div>
    );
  }
    ...
    <>
      <div className="flex items-center justify-between border-b border-eclipse-border/70 px-5 py-2 text-xs">
        <span className="text-eclipse-muted">
          {count.toString()} epoch{count === 1n ? "" : "s"} committed
        </span>
        <span
          className={cn(
            "font-medium",
            verifyChain.data ? "text-eclipse-teal" : "text-eclipse-danger",
          )}
        >
          ...
          {rows.map((e, i) => {
            ...
            return (
              <tr key={e.hash} className="border-t border-eclipse-border/70">
                ...
                <td className="px-3 py-2 text-eclipse-muted">
                  {new Date(Number(e.timestamp) * 1000).toLocaleString()}
                </td>
                <td className="px-3 py-2 text-right text-eclipse-text">
                  {assetDecimals.data !== undefined ? formatUnits(e.nav, assetDecimals.data) : "…"}
                </td>
  ...
  return (
    <div className="glass-card sticky top-24 p-5">
      ...
      {isPaused && (
        <div className="mb-4 rounded-lg border border-eclipse-danger/40 bg-eclipse-danger/10 p-3 text-xs text-eclipse-text">
          ...
          <p className="mt-1 text-eclipse-muted">
            AlphaVault's circuit breaker is currently tripped (
            <code className="text-eclipse-purple">paused()</code> returns true). Deposits and
            redemptions are disabled on-chain until the strategist resets it.
          </p>
        </div>
      )}
      ...
      {!isConnected ? (
        <div className="flex flex-col items-center gap-3 py-6 text-center">
          <p className="text-xs text-eclipse-muted">
            Connect a Coston2 wallet to {tab.toLowerCase()}.
          </p>
          <WalletButton />
        </div>
      ) : (
        <>
          <div className="flex items-center justify-between text-xs text-eclipse-muted">
            ...
            <button onClick={setMax} className="text-eclipse-purple hover:underline">
              Balance: {fmtToken(maxRaw, decimals)}{" "}
      ...
      <p className="mt-3 text-[11px] text-eclipse-muted">
        Deposits call AlphaVault's ERC-4626 <code className="text-eclipse-purple">deposit()</code>{" "}
        directly on Coston2. Approval is requested first if the current allowance is insufficient.
      </p>
    </div>
  );
}
